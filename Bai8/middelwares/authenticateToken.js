...
const authenticationToken = (req, res, next) => {
 ...
 const token = authHeader && authHeader.split(' ')[1];
    ...
    jwt.verify(token, process.env.JWT_SECRET, async (err, decodedPayload) => {
        if (err) {
            if (err instanceof jwt.TokenExpiredError) { // loi het han hau khong 
        ...
        // neu ma thuc hien duoc se truy van vao database 
        ...
        try {
            ...
            console.error("loi try van nguoi dung trong authenticationToken", dbError);
            next(dbError);
        }
    })
